...
export default function ManualGrantsPage() {
  const [users, setUsers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/superadmin/premium?filter=manual&limit=100")
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Manual Premium Grants</h2>
          <p className="text-sm text-muted-foreground">{users.length} manually granted premiums</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-8 text-center text-gray-500">No manual grants found</CardContent></Card>
      ) : (
        <Card>
              ...
              <tbody>
                {users.map((g: any) => (
                  <tr key={g.id} className="border-b hover:bg-gray-50/50">
                    ...
                    <td className="p-3">
                      <Badge variant={g.isActive ? "success" : "secondary"} className="text-[10px]">
