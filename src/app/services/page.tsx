...
function ServicesContent() {
  ...
  const canReuseStoredMapCoords = location.trim().length > 0
    && location.trim().toLowerCase() === search.location.trim().toLowerCase()
    && search.locationLat !== null
    && search.locationLng !== null;
  const mapSearchHref = (() => {
    const params = new URLSearchParams();
    if (location) params.set("location", location);
    if (canReuseStoredMapCoords) {
      ...
      <div className={cn(
        ...
        <div className="hidden lg:flex items-center gap-3 px-6 py-2.5 lg:max-w-6xl xl:max-w-7xl mx-auto">
          ...
          <div className="flex-1 flex items-center gap-2 bg-white border-2 border-gray-200 rounded-full px-4 py-2.5 shadow-lg hover:shadow-xl transition-shadow ml-6">
            ...
            <Link href={mapSearchHref} className="h-9 w-9 rounded-full border-2 border-gray-200 flex items-center justify-center hover:bg-primary/10 hover:border-primary/30 transition-colors shrink-0" title="Search on Map"><Map className="h-4 w-4 text-gray-500" /></Link>
            <div className="h-8 w-px bg-gray-200 shrink-0" />
